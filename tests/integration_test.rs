...
fn test_integration_basic_setup() {
    ...
    // Create a BusyCrab instance with minimal settings
    let busycrab = BusyCrab::new(1, 1);
    
    // Test that it can be configured
    let busycrab = busycrab.with_verbose(true);
    assert!(busycrab.is_verbose());
...
fn test_system_interaction() {
    ...
    // Create a BusyCrab instance with a short interval and tiny wiggle
    let mut _busycrab = BusyCrab::new(1, 1);
